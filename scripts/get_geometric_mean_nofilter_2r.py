...
#This block loops through lines of infile and gets geometric mean between histone and dnase rpms
with open(infile) as f:
        inlines = f.readlines()
        for line in inlines:
            line_list = line.strip().split("\t")
            if line_list[0] == "Geneid":
		#This block adds to header
                out_list = line_list[:4] + ["H3K27ac_comb_rpm","dnase_comb_rpm","geometric_mean"]
                out.writerow(out_list)
            else:
		#This block gets geometric mean and writes to out file 
